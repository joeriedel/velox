...
# CMake >= 4.0, required by the bundled UCXX build. CMake/resolve_dependency_modules/ucxx.cmake
# pins UCXX 0.51 with rapids-cmake 26.08, whose top-level CMakeLists declares
# `cmake_minimum_required(VERSION 4.0)` -- configuring with anything older
# fails outright. Rocky 9's appstream cmake tops out at 3.31.8, so this comes
# from pip, which installs to /usr/local/bin and therefore takes precedence
# over /usr/bin/cmake on the default PATH.
#
# Installed here rather than at container-start time because the container is
# ephemeral (`--rm`), so a runtime `pip install` is lost on every stop.
#
# CMAKE_POLICY_VERSION_MINIMUM=3.5 is already set above, which is the other
# half of running CMake 4 against this dependency set: it keeps dependencies
# that still declare pre-3.5 minimums configurable.
