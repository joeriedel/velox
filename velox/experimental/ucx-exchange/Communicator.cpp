...
int maxWorkItemsPerDrain() {
  static const int value =
      readIntEnv("VELOX_UCX_MAX_WORK_ITEMS_PER_DRAIN", 256, 1, 1 << 20);
  return value;
}

bool isSharedMemoryTransport(std::string_view transport) {
  return transport == "posix" || transport == "sysv" || transport == "cma" ||
      transport == "xpmem" || transport == "knem" || transport == "mm";
}
...
Communicator::createSameHostEndpointRefFromWorkerAddress(
  ...
  try {
    ...
    LOG(WARNING) << "CPU same-host UCX endpoint closed before close callback "
                    "registration: "
                 << e.what();
    return nullptr;
  }
